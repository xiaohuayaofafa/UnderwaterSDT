...
class DetLocalVisualizer(Visualizer):
    ...
    def __init__(self,
                 ...
                 alpha: float = 0.8,
                #  加了下面两行
                 pred_bbox_color: Optional[Union[str, Tuple[int]]] = None,
                 gt_bbox_color: Optional[Union[str, Tuple[int]]] = None) -> None:
        super().__init__(
            name=name,
            image=image,
        ...
        self.alpha = alpha
        # 加了下面两行
        self.pred_bbox_color = pred_bbox_color
        self.gt_bbox_color = gt_bbox_color
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances and instances.bboxes.sum() > 0:
            print(f"Detected bboxes: {instances.bboxes}")  # 添加调试信息
            ...
            # 根据是真值还是预测值选择不同的颜色
            if 'scores' in instances:  # 预测框
                bbox_color = self.pred_bbox_color
            else:  # 真值框
                bbox_color = self.gt_bbox_color
    ...
    def add_datasample(
        ...
        if draw_gt and data_sample is not None:
            ...
            if 'gt_instances' in data_sample:
                ...
                if gt_instances.bboxes.sum() == 0:
                    print("Warning: gt_instances bboxes are empty.")
